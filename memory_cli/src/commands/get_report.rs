...
fn validate_get_report(db_path: &str, schema_path: Option<&str>) -> ValidationResult {
...
pub fn handle(
    ...
    let validation = validate_get_report(db_path, schema_path);
    ...
    let resp = store.get_report_by_cluster_pk_hash(cluster_pk_hash)?;
