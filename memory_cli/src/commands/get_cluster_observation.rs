...
fn validate_get_cluster_observation(period_start: &str, period_end: &str, db_path: &str, schema_path: Option<&str>) -> ValidationResult {
...
pub fn handle(dry_run: bool, db_path: &str, schema_path: Option<&str>, source: &str, period_start: &str, period_end: &str) -> Result<()> {
    let validation = validate_get_cluster_observation(period_start, period_end, db_path, schema_path);
    ...
    let resp = store.get_cluster_observation(source, period_start, period_end)?;
