...
def init_report_job(cluster_pk_hash: str, cfg: ReportGenerationConfig) -> InitReportJobResponse:
    ...
    try:
        ...
        if job is None:
            ...
            logger.info(f"{log_prefix}New job is running.")
        ...
        if status == ReportJobStatus.RUNNING:
            ...
            if updated_at_str:
                try:
                    ...
                    pass
            logger.info(f"{log_prefix}An existing job is already running.")
            return InitReportJobResponse(
                next_status="waiting",
                meta=InitReportJobResponseMeta(
            ...
            if updated_at_str:
                try:
                    ...
                    pass
            logger.info(f"{log_prefix}Job is done before.")
            return InitReportJobResponse(
                next_status="done",
                meta=InitReportJobResponseMeta(
                    last_update_utc=updated_at_str,
            ...
            if elapsed < five_minutes:
                ...
                if updated_at_str:
                    try:
                        ...
                        pass
                logger.info(f"{log_prefix}Job has error, waiting for expiration")
                return InitReportJobResponse(
                    next_status="waiting",
                    meta=InitReportJobResponseMeta(
                        last_update_utc=updated_at_str,
                ...
                logger.info(f"{log_prefix}Error expired, job ready to resume.")
                return InitReportJobResponse(
